!function() {
    ...
    var Consts = require("../consts"), AwdString = require("../types/awdString"), UserAttr = require("../types/userAttr"), Properties = require("../types/properties"), BaseElement = require("../BaseElement"), Geometry = BaseElement.createStruct(Consts.GEOMETRY, null, {
        ...
        write: function(writer) {
            var subGeoms = this.subGeoms, sgLen = subGeoms.length;
            AwdString.write(this.name, writer), writer.U16(sgLen);
            var geomType = this.awd.header.geoNrType, props = this.props;
            props.expected[1] = geomType, props.expected[2] = geomType, props.set(1, 1), props.set(2, 1), 
            props.write(writer);
            for (var subGeom, i = 0; sgLen > i; i++) subGeom = subGeoms[i], subGeom.write(this.awd, writer);
    ...
    SubGeom.prototype = {
        ...
        read: function(awd, reader) {
            ...
            -1 === nverts && console.log("Error, Can't resolve geom buffers sizes");
