...
var BaseElement = require("../BaseElement"), UserAttr = require("../types/userAttr"), Consts = require("../consts"), ExtInfos = require("./extInfos"), Container = require("./Container"), SH_TYPE_TRANSPOSED = 0, SH_TYPE_REGULAR = 1, Env = BaseElement.createStruct(ExtInfos.OPTX_ENV, ExtInfos.URI, {
    ...
    write: function(writer) {
        ...
        if (this.writeNodeCommon(writer), writer.U32(envMap_id), !this.shCoefs) throw new Error("Env have no sh");
        writer.U8(getSHTypeForLength(this.shCoefs.length));
        for (var i = 0; i < this.shCoefs.length; i++) writer.F32(this.shCoefs[i]);
        writer.F32(this.brightness), this.extras.write(writer);
    },
