...
var Material = BaseElement.createStruct(ExtInfos.OPTX_MATERIAL, ExtInfos.URI, {
    ...
    write: function(writer) {
        AwdString.write(this.name, writer), this.writeTexture(this.textures.albedo, writer), 
        this.writeTexture(this.textures.reflectivity, writer), this.writeTexture(this.textures.normal, writer), 
        this.writeTexture(this.textures.subsurface, writer), this.writeTexture(this.textures.agt, writer), 
        this.writeColor(this.colors.albedo, writer), this.writeColor(this.colors.reflectivity, writer), 
        this.writeColor(this.colors.normal, writer), this.writeColor(this.colors.subsurface, writer), 
    ...
    writeTexture: function(tex, writer) {
        tex ? writer.U32(tex.chunk.id) : writer.U32(0);
    },
    setupProps: function(props) {
        props.set(kP_blend, this.blend), props.set(kP_alphaThreshold, this.alphaThreshold), 
        props.set(kP_dithering, this.dithering), props.set(kP_fresnel, this.fresnel), props.set(kP_horizonOcclude, this.horizonOcclude), 
        this.vertexColor && (props.set(kP_vertexColor, this.vertexColor), props.set(kP_vertexColorAlpha, this.vertexColorAlpha), 
        props.set(kP_vertexColorSRGB, this.vertexColorSRGB), props.set(kP_aniso, this.aniso)), 
        this.aniso && (props.set(kP_anisoStrength, this.anisoStrength), props.set(kP_anisoIntegral, this.anisoIntegral), 
        props.set(kP_anisoTangent, this.anisoTangent), props.set(kP_subsurface, this.subsurface)), 
        this.subsurface && (props.set(kP_subsurfaceColor, this.subsurfaceColor), props.set(kP_transColor, this.transColor), 
        props.set(kP_fresnelColor, this.fresnelColor), props.set(kP_fresnelOcc, this.fresnelOcc), 
        props.set(kP_fresnelGlossMask, this.fresnelGlossMask), props.set(kP_transSky, this.transSky), 
        props.set(kP_shadowBlur, this.shadowBlur), props.set(kP_normalSmooth, this.normalSmooth)), 
    ...
    getDependencies: function() {
        var res = [], texs = this.textures;
        return texs.albedo && res.push(texs.albedo), texs.reflectivity && res.push(texs.reflectivity), 
        texs.agt && res.push(texs.agt), texs.normal && res.push(texs.normal), texs.subsurface && res.push(texs.subsurface), 
        res;
    },
    toString: function() {
